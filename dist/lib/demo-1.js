(function(){var width,height,largeHeader,canvas,ctx,points,target,animateHeader=true;initHeader();initAnimation();addListeners();function initHeader(){width=window.innerWidth;height=810;target={x:width/2,y:height/2};largeHeader=document.getElementById('large-header');largeHeader.style.height=810+'px';canvas=document.getElementById('demo-canvas');canvas.width=width;canvas.height=810;ctx=canvas.getContext('2d');points=[];for(var x=0;x<width;x=x+width/20){for(var y=0;y<height;y=y+height/20){var px=x+Math.random()*width/20;var py=y+Math.random()*height/20;var p={x:px,originX:px,y:py,originY:py};points.push(p);}}
    ...
    function addListeners(){if(!('ontouchstart'in window)){document.getElementById("box").addEventListener('mousemove',mouseMove);}}
    ...
    function resize(){width=window.innerWidth;height=810;largeHeader.style.height=height+'px';canvas.width=width;canvas.height=height;}
